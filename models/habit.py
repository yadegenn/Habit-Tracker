...
class HabitModel(db.Model):
    ...
    weekly_done = db.Column(db.Integer, nullable=True)
    ...
    def __str__(self):
        return f"id: {self.id} name: {self.name} type: {self.type} is_complete: {self.is_complete} streak: {self.streak} weekly_goal: {self.weekly_goal} created_at: {self.created_at} user_id: {self.user_id} user: {self.user}"